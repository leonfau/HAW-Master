...
public final class NodeImpl extends Node {
	...
	NodeImpl(ChordImpl impl, ID nodeID, URL nodeURL,
		...
		/*
		 * if (impl == null || nodeID == null || nodeURL == null || references
		 * == null || entries == null || nodeCallback == null) { throw new
	...
	public final void broadcast(Broadcast info) throws CommunicationException {
		...
		for (int i = 0; i <= fTable.size() - 1; i++) {
			...
			if (!receiver.getNodeID().equals(impl.getID())
				...
				// Muss transactionID geÃ¤ndert werden? // Ich glaube die wird nur von ChordImpl hochgezählt
